...
const getTeacherStats = async (req, res) => {
    try {
        ...
        // Get tests count and total hours from teacher's data
        // You can modify this based on your actual data structure
        const testsCount = teacher.tests ? teacher.tests.length : 0;
        const totalHours = teacher.totalHours || 0;

        res.status(200).json({ testsCount, totalHours });
    } catch (error) {
        res.status(500).json({ message: "Error getting teacher stats" });
    }
};
